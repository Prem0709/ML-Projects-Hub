...
models = {name: pickle.load(open(f'C:/Users/pawar/data_science/ML/classification/House/pkl/{name}.pkl', 'rb')) for name in model_names}

# Load evaluation results
# results_df = pd.read_csv('model_evaluation_results.csv')
results_df = pd.read_csv(
    'C:/Users/pawar/data_science/ML/classification/House/model_evaluation_results.csv'
)
...
@app.route('/results')
def results():
    tables = [results_df.to_html(classes='table table-striped table-hover table-bordered', index=False)]
    return render_template('model.html', tables=tables)
